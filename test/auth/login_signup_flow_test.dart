...
void main() {
  ...
  testWidgets('회원가입 화면에서 비밀번호 확인이 다르면 오류가 표시되고 가입되지 않는다', (
    ...
    await tester.enterText(find.byType(TextField).at(2), 'newuser@ason.app');
    await tester.enterText(find.byType(TextField).at(3), 'pw1234');
    await tester.enterText(find.byType(TextField).at(4), 'different');
  ...
  testWidgets('회원가입 화면에서 필수 약관에 동의하지 않으면 오류가 표시된다', (tester) async {
    ...
    await tester.enterText(find.byType(TextField).at(2), 'newuser@ason.app');
    await tester.enterText(find.byType(TextField).at(3), 'pw1234');
    await tester.enterText(find.byType(TextField).at(4), 'pw1234');
    await tester.tap(find.text('가입하기'));
    await _settle(tester);
  ...
  testWidgets('로그인 화면에서 회원가입을 마치면 로그인 화면으로 돌아와 완료 안내가 보인다', (
    ...
    await tester.enterText(find.byType(TextField).at(3), 'pw1234');
    await tester.enterText(find.byType(TextField).at(4), 'pw1234');
    await tester.tap(find.text('[필수] 이용약관 및 개인정보 처리방침에 동의합니다.'));
    await _settle(tester);
    await tester.tap(find.text('가입하기'));
    await _settle(tester);
