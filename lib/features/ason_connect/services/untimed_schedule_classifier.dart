...
class UntimedScheduleClassifier {
  ...
  // "확인 중", "~할지 고민이다"). 확정된 용무가 아니라 아직 검토 중인
  // 내용이므로 보너스를 주지 않습니다.
  static final RegExp _questionOrUncertainPattern = RegExp(
    r'[?？]|(까|나요|뭐지|뭘까)\s*$|확인\s*중|검토\s*중|고민',
  );
  ...
  // 표현입니다("병원 가야 해", "은행에 서류 제출", "주민센터 방문",
  // "보험 갱신", "엄마한테 전화" 등). "-야 해/한다/겠/해요/합니다"(의무형)는
  // "가다" 말고 "보내다"에도 흔히 붙어서 함께 인식합니다("택배 좀 보내야
  // 하는데"). "OO한테/에게/께 전화"는 동사 없이 명사만 말하는 음성 입력에서
  // 아주 흔한 형태입니다.
  static final RegExp _errandWordPattern = RegExp(
    r'(가야|보내야)\s*(해|한다|겠|해요|합니다|하는데)|방문|제출|예약|신청|'
    r'확인하러\s*가|갱신|[가-힣]+(한테|에게|께)\s*전화(?!하)',
  );

  // 발신·수령·위탁처럼 한 번 처리하면 끝나는 용무 동사로 문장이 끝나는
  // 경우입니다("세차 맡기기", "어머니께 전화하기", "택배 보내기",
  // "자동차 검사받기", "세탁소 들르기", "보험 갱신하기", "은행 갔다 오기" 등).
  static final RegExp _errandEndingPattern = RegExp(
    r'(맡기기|찾아오기|구매하기|전화하기|신청하기|제출하기|예약하기|보내기|받기|'
    r'들르기|갱신하기|갔다\s*오기)\s*[.!?]*$',
  );
  ...
  /// 아직 결정되지 않은 질문·고민 표현입니다("~필요할까?", "~할지 고민이다"
  /// 등). "서류를 제출해야 하나?"처럼 메모의 구매/준비 동사 패턴(사야/해야
  /// 등)과 우연히 겹치는 경우에도, 이 신호가 있으면 확정된 메모로 보지
  /// 않습니다. isUnlikelyToBeSchedule보다 좁은 범위(질문/고민만)라서 "OO
  /// 기록"처럼 메모 자체는 정당하게 점수를 받아야 하는 경우까지 막지
  /// 않습니다.
  bool isQuestionOrUncertain(String text) {
    return _questionOrUncertainPattern.hasMatch(text);
  }
