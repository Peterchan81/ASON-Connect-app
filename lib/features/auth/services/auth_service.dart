...
class AuthService {
  ...
  /// 지금 로그인된 계정의 전체 정보(닉네임/아이디/이메일)를 가져옵니다.
  /// 로그인 상태가 아니면 null입니다.
  Future<AsonAccount?> currentAccount() async {
  ...
  /// 닉네임만 변경합니다. (아이디/이메일은 설정 화면에서 변경할 수 없습니다)
  Future<AuthResult> updateNickname(String nickname) async {
    final userId = _currentUserId;
    if (userId == null) {
      return const AuthResult.failure('로그인이 필요합니다.');
    }

    final trimmed = nickname.trim();
    if (trimmed.isEmpty) {
      return const AuthResult.failure('닉네임을 입력해주세요.');
    }

    final accounts = await _loadAccounts();
    final index = accounts.indexWhere((a) => a.id == userId);
    if (index == -1) {
      return const AuthResult.failure('계정 정보를 찾을 수 없습니다.');
    }

    accounts[index] = AsonAccount(
      nickname: trimmed,
